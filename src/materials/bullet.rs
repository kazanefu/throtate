use bevy::prelude::*;
use bevy::render::render_resource::*;
use bevy::shader::ShaderRef;
use bevy::sprite_render::{AlphaMode2d, Material2d, Material2dPlugin};

pub struct BulletMaterialPlugin;

impl Plugin for BulletMaterialPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(Material2dPlugin::<BulletMaterial>::default())
            .add_systems(Update, update_bullet_material);
    }
}

#[derive(ShaderType, Clone, Copy, Debug, Default)]
pub struct BulletUniform {
    pub time: f32,
}

#[derive(Asset, TypePath, AsBindGroup, Debug, Clone, Default)]
pub struct BulletMaterial {
    #[uniform(0)]
    pub params: BulletUniform,
}

impl Material2d for BulletMaterial {
    fn fragment_shader() -> ShaderRef {
        "embedded://throtate/shaders/bullet.wgsl".into()
    }

    fn alpha_mode(&self) -> AlphaMode2d {
        AlphaMode2d::Blend
    }
}

fn update_bullet_material(
    time: Res<Time>,
    mut materials: ResMut<Assets<BulletMaterial>>,
    query: Query<&MeshMaterial2d<BulletMaterial>>,
) {
    for handle in &query {
        if let Some(material) = materials.get_mut(handle) {
            material.params.time = time.elapsed_secs();
        }
    }
}
